...
const ICONS = {
  ...
  // ── UI-only icons (not shown in category picker) ──
  pencil:     `<path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z"/>`,
  trash:      `<polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/><path d="M9 6V4a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v2"/>`,
};

// Only category-themed icons appear in the picker; UI icons are excluded
...
function save(){ localStorage.setItem('tf_v3',JSON.stringify(state)); }
function load(){ const s=localStorage.getItem('tf_v3'); if(s) try{ state=JSON.parse(s); }catch(e){} }

function exportJson(){
  const data = { categories: state.categories, projects: state.projects };
  const blob  = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' });
  const a     = Object.assign(document.createElement('a'), { href: URL.createObjectURL(blob), download: 'taskflow-export.json' });
...
function handleJsonFile(input){
  ...
  reader.onload = e => {
    try {
      ...
      alert('Could not load file — make sure it is a valid TaskFlow JSON export.');
    }
  };
...
function renderSidebar(){
  ...
  if(!state.categories.length && !state.projects.length){
    html = `<div style="padding:20px 14px;font-size:12px;color:var(--text-muted);line-height:1.7">
      Start by creating a <strong style="color:var(--text-dim)">category</strong> (e.g. Engineering, Marketing) then add <strong style="color:var(--text-dim)">projects</strong> inside it.
    </div>`;
  }
...
function renderOverdue(){
  // Collect all overdue tasks across all projects
  const groups = [];
...
function selectProject(id){
  ...
  state.activeProjectId=id; state.activeTaskId=null; save(); render();
}
function setView(v){ state.view=v; save(); renderMain(); }
function toggleCat(id){ const c=getCat(id); if(c){ c.collapsed=!c.collapsed; save(); renderSidebar(); } }

function addTask(){
  const p=getActive(); if(!p) return;
  const t={id:uid(),title:'New Task',status:'todo',assignee:'',dueDate:'',subtasks:[],comments:[],createdAt:Date.now()};
  p.tasks.unshift(t); state.activeTaskId=t.id;
  save(); renderMain(); renderSidebar(); renderPanel();
...
function updateTaskTitle(val){
  const t=getTask(state.activeTaskId); if(!t) return;
  t.title=val.trim()||'Untitled'; save(); renderMain(); renderSidebar();
}
function updateField(f,v){
  const t=getTask(state.activeTaskId); if(!t) return;
  t[f]=v; save(); renderMain(); renderSidebar();
}
function addSub(){
  const inp=document.getElementById('new-sub'), v=inp?inp.value.trim():''; if(!v) return;
  const t=getTask(state.activeTaskId); if(!t) return;
  t.subtasks.push({id:uid(),title:v,done:false});
  inp.value=''; save(); renderPanel(); renderMain();
}
function toggleSub(sid,done){
  const t=getTask(state.activeTaskId); if(!t) return;
  const s=t.subtasks.find(s=>s.id===sid); if(s) s.done=done;
  save(); renderPanel(); renderMain();
}
function deleteSub(sid){
  const t=getTask(state.activeTaskId); if(!t) return;
  t.subtasks=t.subtasks.filter(s=>s.id!==sid); save(); renderPanel(); renderMain();
}
function addComment(){
  const ne=document.getElementById('c-name'), te=document.getElementById('c-text');
  const author=ne?ne.value.trim()||'Anonymous':'Anonymous';
  const text=te?te.value.trim():''; if(!text) return;
  const t=getTask(state.activeTaskId); if(!t) return;
  t.comments.push({id:uid(),author,text,ts:Date.now()});
  if(te) te.value=''; save(); renderPanel();
}
...
function onDrop(e,col){
  ...
  const t=getTask(dragTaskId); if(t){ t.status=col; save(); renderMain(); renderSidebar(); }
  dragTaskId=null;
}
...
function createCategory(){
  ...
  state.categories.push({id:uid(),name,icon:_icon,collapsed:false});
...
function createProject(){
  ...
  state.projects.push({id:uid(),name,color:_color,categoryId:catId,tasks:[]});
  state.activeProjectId=state.projects[state.projects.length-1].id;
...
function updateProject(id){
  ...
  p.name = name;
  p.color = _color;
  p.categoryId = document.getElementById('edit-proj-cat').value||null;
  save(); closeModal(); render();
}
